...
Dokumen ini menjelaskan arsitektur teknis aplikasi **SkillSwap**, lanjutan dari `README.md`. Fokusnya: bagaimana layer-layer aplikasi disusun, bagaimana data mengalir, dan bagaimana Firebase digunakan sebagai backend.
...
13. [Deployment & Environment Topology](#13-deployment--environment-topology)
14. [Strategi Error Handling](#14-strategi-error-handling)
15. [Reliability & Offline Handling](#15-reliability--offline-handling)
...
* **State management terpusat** — perubahan state (login, list match, chat, dsb) dikelola lewat Provider, bukan `setState` tersebar di banyak widget.
...
│      matching_service, storage_service      │
└──────────────────────┬──────────────────────┘
                       │ read/write
┌──────────────────────▼──────────────────────┐
│                  DATA LAYER                 │
...
**Aturan arah dependency:** panah hanya boleh ke bawah. Screen tidak boleh import Firebase langsung, Provider tidak boleh import widget, dst.
...
Perluasan dari struktur di README, sekarang ditambah folder `providers/`:
...
│
├── core/
│   ├── constants/  # warna, string, ukuran, dsb
│   ├── theme/      # ThemeData Flutter
│   └── utils/      # helper function (format tanggal, validasi, dll)
│
├── models/
│   ├── user_model.dart
│   ├── skill_model.dart
...
│   ├── home/
│   ├── discover/     # skill discovery + matching result
│   ├── request/      # kirim / terima skillswap request
│   ├── chat/
│   ├── session/      # scheduling, detail session
│   └── profile/
│
├── widgets/
│   ├── skill_card.dart
...
|---|---|
| `auth_service.dart` | Register, login, logout, cek status login, reset password |
| `firestore_service.dart` | CRUD generic untuk users, skills, user_skills, requests, sessions, reviews |
| `matching_service.dart` | Hitung skor matching antar pengguna (lihat bagian 7) |
| `storage_service.dart` | Upload & ambil URL foto profil dari Firebase Storage |
...
// matching_service.dart
double calculateMatchScore(UserModel currentUser, UserModel targetUser);
...
sequenceDiagram
    ...
    participant P as DiscoverProvider
    ...
    F->>DB: query users + user_skills
    DB-->>F: list of candidate users
    F-->>P: List<UserModel>
    P->>M: getRankedMatches(currentUser, candidates)
    M-->>P: List<MatchResult> (sorted by score)
    P-->>UI: notifyListeners() -> render match cards
```

### 5.2 Proses SkillSwap Request (Kirim & Terima)
...
sequenceDiagram
    ...
    C->>DB: stream snapshot on messages where conversation_id == id
    DB-->>C: onSnapshot (real-time)
    C-->>P: Stream<List<MessageModel>>
...
### 5.4 Proses Session Scheduling & Completion
...
sequenceDiagram
    ...
    UI->>P: scheduleSession(skill, date, time, mode)
    P->>F: createSession(SessionModel status: upcoming)
    F->>DB: create doc in sessions
    DB-->>F: ok
    F-->>P: success
    P-->>UI: tampilkan jadwal di kalender sesi

    Note over UI,DB: Saat waktu sesi tiba/lewat
    UI->>P: markSessionCompleted(sessionId)
    P->>F: updateSessionStatus(sessionId, "completed")
    F->>DB: update doc status -> completed
    DB-->>F: ok
    F-->>P: success
    P-->>UI: arahkan ke halaman Rating & Review
```

### 5.5 Proses Rating & Review
...
sequenceDiagram
    ...
    F->>DB: create doc in reviews
    F->>DB: update users/{reviewedUserId}.rating (recalculate average)
    DB-->>F: ok
    F-->>P: success
...
### 6.2 Index yang Dibutuhkan

Karena matching butuh filter kombinasi (`skill_id` + `type`), buat composite index:

```text
Collection: user_skills
Fields: skill_id (Ascending), type (Ascending)

Collection: requests
Fields: receiver_id (Ascending), status (Ascending), created_at (Descending)

Collection: sessions
Fields: teacher_id / learner_id (Ascending), status (Ascending), date (Ascending)
```
    ...
    match /sessions/{docId} {
      allow read, write: if request.auth != null
                    && (resource.data.teacher_id == request.auth.uid
                        || resource.data.learner_id == request.auth.uid);
    }
...
> Ini masih outline — perlu disesuaikan lagi saat implementasi (misalnya validasi tipe data, panjang string, dsb).
...
Rules dasar: hanya pemilik akun yang boleh upload/replace foto miliknya sendiri, semua user login boleh membaca.
...
| Komponen | Cara Hitung (sederhana) |
|---|---|
| **Skill Match** | Apakah skill yang ingin dipelajari user A ada di daftar "Can Teach" user B → 100 jika cocok, 0 jika tidak |
| **Reverse Match** | Apakah skill yang ingin dipelajari user B ada di daftar "Can Teach" user A |
| **Schedule Match** | Persentase overlap availability (misal jumlah slot waktu yang sama / total slot) |
| **Mode Match** | Sama-sama online, sama-sama offline, atau sama-sama fleksibel → 100; beda mode → 0 atau 50 |
| **Rating** | Rating rata-rata user B dinormalisasi ke skala 0–100 (misal `rating/5 * 100`) |
...
Menggunakan named routes terpusat di `routes/app_routes.dart`, contoh struktur:
...
Auth guard sederhana: sebelum masuk ke `/home` dan seterusnya, cek `authStateChanges` dari `auth_service` — kalau belum login, redirect ke `/login`.

---

## 9. State: Loading / Empty / Error

Setiap Provider yang mengambil data dari Firestore disarankan punya 3 state standar, supaya UI konsisten di semua screen:
...
if (provider.state == ViewState.error)   → error message + tombol retry
...
Input  getRankedMatches(currentUser, candidates):
  currentUser: UserModel { id, teachSkills[], learnSkills[], availability[], mode, rating }
...
### 12.3 `chat_service.dart` — `listenMessages`
...
### 12.4 Dashboard Summary (untuk Home Screen, jika ditambahkan nanti)
...
### 13.2 Flutter Flavor / Environment Config

```text
lib/
└── core/
    └── config/
        ├── firebase_options_dev.dart     # hasil `flutterfire configure` project dev
        └── firebase_options_prod.dart    # hasil `flutterfire configure` project prod
```
...
graph LR
    ...
    A -->|HTTPS| D[Firebase Storage]
    A -->|HTTPS| E[Firebase Cloud Messaging]
```

Tidak ada server backend custom — semua request dari app langsung ke layanan Firebase (serverless), jadi tidak perlu hosting backend terpisah.

### 13.4 CI/CD (Opsional, untuk pengembangan lanjutan)
...
GitHub push -> GitHub Actions
   ...
   2. flutter analyze
   3. flutter test
   4. flutter build apk --flavor dev
...
|---|---|
| `user-not-found` / `wrong-password` | "Email atau kata sandi salah." |
...
Provider tinggal `try { ... } catch (AppException e) { state = ViewState.error; errorMessage = e.message; }` — tidak perlu tahu detail Firebase sama sekali.

---

## 15. Reliability & Offline Handling

* **Firestore offline persistence** diaktifkan secara default di Flutter — data yang pernah dibaca tetap bisa diakses walau koneksi terputus sementara, dan write akan otomatis di-sync ulang saat online kembali.
...
* **Firestore quota**: karena tier gratis (Spark Plan) punya batas read/write harian, hindari query berulang tanpa perlu (misalnya jangan `get()` ulang di setiap rebuild widget — simpan di Provider state).
* **Race condition sederhana**: saat dua pihak menerima request bersamaan (jarang terjadi di skala kecil), cukup andalkan `status` field di dokumen request sebagai penentu — request yang statusnya sudah berubah dari `pending` akan ditolak otomatis oleh security rules.
